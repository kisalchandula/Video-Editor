    ...
    private static void stopMuxer(MediaMuxer mediaMuxer) {
        try {
            ...
            if (SDK_INT < 30) {
                ...
                try {
                    @SuppressLint("DiscouragedPrivateApi") Field muxerStoppedStateField = MediaMuxer.class.getDeclaredField("MUXER_STATE_STOPPED");
                    muxerStoppedStateField.setAccessible(true);
                    int muxerStoppedState = castNonNull((Integer) muxerStoppedStateField.get(mediaMuxer));
                    @SuppressLint("DiscouragedPrivateApi") Field muxerStateField = MediaMuxer.class.getDeclaredField("mState");
